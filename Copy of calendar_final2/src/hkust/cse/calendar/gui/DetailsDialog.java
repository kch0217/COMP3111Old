...
//		area.setPreferredSize(new Dimension(400, 300));
	...
	public void Display(Appt appt) {
		...
		if( appt.getReminder() != null){
			tempH = appt.TimeSpan().StartTime().getHours() - appt.getReminder().getHours();
			tempM = appt.TimeSpan().StartTime().getMinutes() - appt.getReminder().getMinutes();
			if( tempM < 0){
				tempH--;
				tempM += 60;
			}
		...
		String date = String.valueOf( appt.TimeSpan().StartTime().getYear() + 1900);
		date += "-";
		date += String.valueOf( appt.TimeSpan().StartTime().getMonth() + 1);
		date += "-";
		date += String.valueOf( appt.TimeSpan().StartTime().getDate());
		
		int tempFreq = appt.getFrequency();	// Get the Frequency 
		String Frequency = "";
		switch( tempFreq){
		case 0:	Frequency = "Once Only";
				break;
		...
		area.append("Frequency: " + Frequency + "\n");						// Output the Frequency
		area.append("Initiator: " + appt.getUser() + "\n");
		if( appt.getPublicity() == true){
			area.append("Publicity: Public\n");
		}
		else{
			area.append("Publicity: Private\n");
		}
		if( tempH == 0 && tempM == 0){										// Output the Reminder
			area.append("Reminder: None");							
		}
		else{

			area.append("Reminder: " + tempH + " Hour " + tempM + " Minute before the appointment start.\n");
	...
	public void Display(Vector[] vs, User[] entities) {
		...
		for (int i = 0; i < entities.length; i++) {
			area.append((i + 1) + ". " + entities[i].toString() + " :\n\n");
