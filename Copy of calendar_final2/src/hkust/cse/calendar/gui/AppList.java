...
public class AppList extends JPanel implements ActionListener {
		...
		for (int i = startMin; i < endMin; i = i + SMALLEST_DURATION) {
			...
			if (i == startMin) {
				if (appt.getState()){
					...
					tableView.getModel().setValueAt("Scheduled", pos[0], pos[1]+1);
				}
				else{
					tableView.getModel().setValueAt(appt, pos[0], pos[1]);
					tableView.getModel().setValueAt("Not Scheduled", pos[0], pos[1]+1);
				}
	...
	private void delete() { //by Hin
		Appt apptTitle = getSelectedAppTitle(); //obtain the appointment
		if (apptTitle == null)
			return;
		
		parent.controller.ManageAppt(apptTitle,1); //pass the appointment to the controller
		
		parent.UpdateCal(); //update the view
	...
	private void modify() {
		...
		if (apptTitle == null)
			return;
		InviteDialog inviteDialog = new InviteDialog(parent, parent.controller, apptTitle, false, -1, "Modify");
...
//		setAppDial.setResizable(false);
	...
	private void NewAppt() {
		...
		newAppt.setID(-1); //-1 means not yet valid
		InviteDialog inviteDialog=new InviteDialog(parent, parent.controller, newAppt, true, startTime, "New");
...
//		a.show();
	...
	private void pressResponse(MouseEvent e) {
		...
		if ((e.getModifiers() & InputEvent.BUTTON3_MASK) != 0){  //Kelvin
			//Check the time slot of the selected cell
			int selectedColor = NOT_COLORED;
			int selectedTime = 0;
			if( pressCol != -1 && pressRow != -1){
				
				if( pressCol > -1 && pressCol < 3){
					selectedColor = cellCMD[pressRow][0];
					selectedTime = pressRow * 15 + 480;		// get the selected Time
				}
				else{
					selectedColor = cellCMD[pressRow][1];	
					selectedTime = (pressRow + 20) * 15 + 480;	// get the selected Time
				}
			}	
			Timestamp stamp = parent.getCurrentDate();
			
			stamp.setHours(selectedTime / 60);
			stamp.setMinutes(selectedTime % 60);
			
			//System.out.println(stamp.getYear()+"/"+ stamp.getMonth()+"/"+stamp.getDate()+"/"+stamp.getHours()+"/"+stamp.getMinutes());
			
			TimeSpan span = new TimeSpan( stamp, stamp);
			
			//System.out.println(span.StartTime().getYear());
			
			//System.out.println( hkust.cse.calendar.unit.TimeMachine.outputMonth() + " " + stamp.getMonth());
			
			
			//Get the Items in the Popup Menu
			Component[] menuItem = pop.getComponents();			
			
			//Check if there is event at the selected cell
			//Enable and disable the items
			if( hkust.cse.calendar.gui.Utility.isPastEvent(span)){	// Check the selected grid is past or not
				if( selectedColor == NOT_COLORED){						// If past check have Appt or not
																// If dont have Appt
					menuItem[0].setEnabled(false);	//New
					menuItem[1].setEnabled(false);	//Modify
					menuItem[2].setEnabled(false);	//Delete
					menuItem[4].setEnabled(false);	//Detail
				}
				else{											// If have Appt
					menuItem[0].setEnabled(false);	//New
					menuItem[1].setEnabled(false);	//Modify
					menuItem[2].setEnabled(false);	//Delete
					menuItem[4].setEnabled(true);	//Detail
				}
			}			
			else{														// If not past check have Appt or not
				if( selectedColor == NOT_COLORED){		
					//System.out.println("Good");											// If dont have Appt
					menuItem[0].setEnabled(true);	//New
					menuItem[1].setEnabled(false);	//Modify
					menuItem[2].setEnabled(false);	//Delete
					menuItem[4].setEnabled(false);	//Detail
				}
				else{											// If have Appt
					Timestamp stamp2 = (Timestamp) stamp.clone();
					stamp2.setMinutes(stamp2.getMinutes()+15);
					TimeSpan span2 = new TimeSpan(stamp, stamp2 );
					Appt[] appt = parent.controller.RetrieveAppts(parent.controller.getDefaultUser(), span2);

					if (appt == null){	// this is a public appt
						menuItem[0].setEnabled(true);	//New
						menuItem[1].setEnabled(false);	//Modify
						menuItem[2].setEnabled(false);	//Delete
						menuItem[4].setEnabled(true);	//Detail
					...
					{
						//System.out.println(appt[0].getUser());
						if (appt[0].getUser().equals(parent.controller.getDefaultUser().getUsername())){ //initiator
					
						menuItem[0].setEnabled(false);	//New
						menuItem[1].setEnabled(true);	//Modify
						menuItem[2].setEnabled(true);	//Delete
						menuItem[4].setEnabled(true);	//Detail
